...
class YoloClusterHeadlessNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('center_deadband_norm', 0.35)  # wider: ball often reaches cy~640 before leaving frame
        self.declare_parameter('align_center_offset_norm', 0.4)  # camera upper-left: shift center right
        ...
        self.align_center_offset_norm = float(self.get_parameter('align_center_offset_norm').value)

        # Frame metadata (from Image header)
        self._latest_frame_id = "camera"
    ...
    def _parse_rotation_from_frame_id(self, frame_id: str) -> int:
        """Parse rotation from frame_id (e.g. 'camera_270' -> 270)."""
    ...
    def process_latest(self):
        ...
        rotation = self._parse_rotation_from_frame_id(frame_id)
        ...
        # Align on cx (horizontal in displayed image). For 270° frame 720x1080, ball was leaving on cx axis.
        horiz_dim = w
        center = horiz_dim / 2

        def pos_norm_for(cx: float, cy: float) -> float:
            return (cx - center) / (horiz_dim / 2) + self.align_center_offset_norm if horiz_dim > 0 else 0.0

        cluster_infos: List[Tuple[int, float, float, float]] = []
        for idxs in cluster_indices:
            cx, cy = cluster_centroid(centers, idxs)
            pn = pos_norm_for(cx, cy)
            cluster_infos.append((len(idxs), abs(pn), cx, cy))

        # Major cluster (most balls, or closest to effective center if tied)
        major = None
        ...
        if major is None:
            ...
            pos_norm = pos_norm_for(major_cx, major_cy)
            if abs(pos_norm) <= self.center_deadband_norm:
                major_side = 0.0
            elif pos_norm < 0:
                major_side = -1.0
            else:
